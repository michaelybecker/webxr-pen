import { VertexColors, DoubleSide, MeshStandardMaterial } from "three";
/* globals AFRAME */
...
(function () {
  ...
  onLoaded(function () {
    ...
    sharedBufferGeometryManager.addSharedBuffer(
      "strip-flat",
      new MeshBasicMaterial(optionsBasic),
      TriangleStripDrawMode
    );
    sharedBufferGeometryManager.addSharedBuffer(
      "strip-shaded",
      new MeshStandardMaterial(optionsStandard),
      TriangleStripDrawMode
    );
    sharedBufferGeometryManager.addSharedBuffer(
      "strip-textured",
      new MeshStandardMaterial(optionTextured),
      TriangleStripDrawMode
    );
  });
  ...
  var line = {
    init: function (color, brushSize) {
      this.sharedBuffer = sharedBufferGeometryManager.getSharedBuffer(
        "strip-" + this.materialOptions.type
      );
    ...
    computeStripVertexNormals: (function () {
      ...
      return function () {
        var start =
          this.prevIdx.position === 0 ? 0 : (this.prevIdx.position + 1) * 3;
        ...
        the rest of the vertices belongs to triangles
        ...
        */
        ...
        // Vertices that are shared across triangles
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (definition.materialOptions.textureSrc) {
      definition.materialOptions.converter = window.atlas.getUVConverters(
        definition.materialOptions.textureSrc
      );
    } else {
    ...
    AFRAME.registerBrush(
      definition.name,
      Object.assign({}, line, { materialOptions: definition.materialOptions }),
      { thumbnail: definition.thumbnail, maxPoints: 3000 }
    );
  }
})();
