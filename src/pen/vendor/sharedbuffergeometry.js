...
SharedBufferGeometry.prototype = {
    ...
    if (this.idx.position > idx.position) {
      for (key in this.idx) {
        ...
        for (var i = start; i < end; i++) {
          this.current.attributes[key].array[pos++] = this.current.attributes[
            key
          ].array[i];
        }
      }
  ...
  addBuffer: function (copyLast) {
    ...
    geometry.addAttribute(
      "uv",
      new THREE.BufferAttribute(uvs, 2).setDynamic(true)
    );
    geometry.addAttribute(
      "normal",
      new THREE.BufferAttribute(normals, 3).setDynamic(true)
    );
    geometry.addAttribute(
      "color",
